...
const APPEAR_ANIMATION_TIME = 150;
...
const getGuessTemperatureTemplate = (cityInfo, results) => (
  `<div class="guess">
    ...
    <div class="guess__result">
      <div class="real">True answer: <span class="bold">${results.real}°C</span></div>
      <div class="user">Your answer: <span class="bold">${results.user}°C</span></div>
      <div class="diff">Difference: <span class="bold" style="color:${Math.abs(results.diff) <= 5 ? 'green' : Math.abs(results.diff) <= 10 ? 'orange' : 'red'}">${results.diff >= 0 ? `+${results.diff}` : String(results.diff)}</span>°C</div>
    </div>
